...
const UpdateNewsModal: React.FC<UpdateNewsModalProps> = ({
  ...
  const { mutate: updatePost, isUpdating } = useUpdatePost();
  ...
  useEffect(() => {
    if (post && isOpen) {
      reset({
        title: post.title,
        slug: post.slug,
        category: post.category as "News" | "Announcement",
  ...
  const onSubmit = (data: FormValues) => {
    ...
    if (data.cover && typeof data.cover !== "string") {
      updateData.coverImage = data.cover;
    }

    if (data.gallery.some((img) => img instanceof File)) {
      updateData.galleryImages = data.gallery;
    }

    updatePost(updateData, {
      onSuccess: (updatedPost) => {
        message.success("Post uğurla yeniləndi!");
        handleClose();
      },
      onError: (error) => {
        console.error(" Yeniləmə xətası:", error);
        message.error("Post yenilənərkən xəta baş verdi!");
      },
    });
  };
  ...
  const handleGalleryUpload = (file: RcFile): boolean => {
  ...
  const getCoverFileList = (): UploadFile[] => {
